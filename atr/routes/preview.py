...
@routes.committer("/refine/<project_name>/<version_name>", methods=["GET", "POST"])
async def refine_release(
    ...
    return await quart.render_template(
        "preview-refine-release.html", release=release, file_paths=sorted(file_paths_rel), form=form, can_move=can_move
    )
        ...
        try:
            async with revision.create_and_manage(project_name, version_name, session.uid, preview=True) as (
                ...
                if await aiofiles.os.path.exists(target_path_in_new):
                    ...
                    return await session.redirect(refine_release, project_name=project_name, version_name=version_name)
